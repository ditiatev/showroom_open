...
WHERE ds BETWEEN 'Repl_start_date' AND 'Repl_end_date'")
  ...
  df_sheck <- dbGetQuery(con_postgre,request_code)
...
WHERE (type = 'Repl_ipc_type') AND (prediction = Repl_prediction)")
  ...
  df_ipc <- dbGetQuery(con_postgre, request_code)
...
get_weather_sql <- function(con_postgre) {
  ...
  df_weather <- dbGetQuery(con_postgre, request_code)
...
get_n_weeks_sql <- function(con_postgre) {
  ...
  df_n_weeks_sgl      <- dbGetQuery(con_postgre, request_code)
  df_n_weeks_sgl$date <- as.Date(df_n_weeks_sgl$date)
  
  return(df_n_weeks_sgl)
}

get_disasters_sql <- function(con_postgre) {
  ...
  df_disasters_sql      <- dbGetQuery(con_postgre, request_code)
  df_disasters_sql$date <- as.Date(df_disasters_sql$date)
  
  return(df_disasters_sql)
}

get_hdays_sql <- function(con_postgre) {
  ...
  df_hdays_sql    <- dbGetQuery(con_postgre, request_code)
  df_hdays_sql$ds <- as.Date(df_hdays_sql$ds)
  
  return(df_hdays_sql)
}

get_plans_sql <- function(con_postgre,
  ...
  df_pred           <- dbGetQuery(con_postgre,request_code)
